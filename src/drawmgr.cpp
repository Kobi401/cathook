...
{
    {
        ...
        if (info_text)
        {
            ...
            color.a    = 1.0f;
            AddSideString("Simplex", color);
            if (!info_text_min)
            {
                AddSideString("Latest Build: Apr 9 2021",  colors::gui);
                // github commit and date, Not Being used now due to cathook auto changing.
                AddSideString(hack::GetType(), colors::gui);
#if ENABLE_GUI
                AddSideString("Better cathook", colors::gui);
                AddSideString("Developed by Kobi401", colors::gui);
                AddSideString("Help from Spice", colors::gui);
#endif
            }
        }
    }
