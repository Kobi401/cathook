...
static settings::Boolean minigun_silent{ "aimbot.auto.minigun-silent", "false" };
...
float cur_proj_speed{ 1980.0f };
float cur_proj_grav{ 800.0f };
...
static void CreateMove()
{
 ...
 else
    {
        if (g_pLocalPlayer->weapon()->m_iClassID() != CL_CLASS(CTFMinigun))
            Aim(target_entity);
        if (!minigun_silent || g_pLocalPlayer->weapon()->m_iClassID() != CL_CLASS(CTFMinigun))
            DoAutoshoot(target_entity);
        if (minigun_silent)
        {
            // Used to keep track of what tick we're in right now
        static int shot_delay = 0;
        shot_delay++;
